...
Pulls a tiny static ``hu-runtime.json.gz`` from this repo's GitHub Release on
first tool call so a dead endpoint can be repointed (edit runtime/hu-runtime.json
...
Opt-out: set ``HU_ELI_RUNTIME_URL=""`` to disable the fetch.
"""
...
MANIFEST_URL = os.environ.get(
    "HU_ELI_RUNTIME_URL",
    "https://github.com/matematicsolutions/hu-eli-mcp/releases/latest/download/hu-runtime.json.gz",
)
_CACHE_NAME = "hu-runtime.json"
_TIMEOUT = 15
_USER_AGENT = "hu-eli-mcp-runtime (+https://github.com/matematicsolutions/hu-eli-mcp)"

_runtime: dict[str, Any] | None = None


def _cache_dir() -> Path:
    env = os.environ.get("HU_ELI_CACHE_DIR")
    if env:
        return Path(env).expanduser()
    return Path.home() / ".matematic" / "cache" / "hu-eli"
