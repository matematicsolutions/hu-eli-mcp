...
from hu_eli_mcp.coverage import build_coverage
from hu_eli_mcp.server import hu_coverage


def _coverage():
    fn = getattr(hu_coverage, "fn", hu_coverage)
...
def test_tool_writes_an_audit_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("HU_ELI_AUDIT_DIR", str(tmp_path))
    _coverage()
    lines = [
        line
    ...
    assert any("hu_coverage" in line for line in lines), (
        "an audit entry was written but it does not name hu_coverage"
    )
